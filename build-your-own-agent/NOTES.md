...
| 文件 | 说明 |
|---|---|
| `index.html` | **主交付**。26 页自包含演示。`← →` 翻页 · `O` 总览 · `F` 全屏 · 点击左右半屏也能翻 |
| `如何打造自己的Agent.pdf` | 26 页 PDF（1280×720，深色保真）—— 改完 slides 跑 `python3 export_pdf.py` 重导 |
...
CH3 原理 P11–14 ├ 扉页 · 一个类比 · 术语对照 · 五个决定
CH4 场景 P15–16 ├ 扉页 · 场景矩阵
CH5 全景 P17–18 ├ 扉页 · coding agent 四象限
CH6 样本 P19–21 ├ 扉页 · pi 的不做清单 · oh-my-pi 与 FORK 成本模型
CH7 实践 P22–25 └ 扉页 · 模型对照 · 选型依据 · 本讲收口与后三讲预告
致谢     P26     谢谢 · Discord / 小红书 / 仓库
```
...
| 呼应 | 怎么用 |
|---|---|
| **P12 → P21** | P12 讲那场受控实验时**先不点名作者**，只说「有人做了个实验」；到 P21 揭晓——做实验的就是把 pi fork 成怪物的 Can Bölük |
| **P10 → P21 → P25** | 人物谱里的 can1357 是写 VMProtect 去虚拟化器的逆向工程师 → 所以 pi 这条线天然通向逆向 → re-agent 不是硬拗的 |
| **P20 → P25** | slogan `this one is yours` 开在 pi 章，收在 re-agent 章 |
| **P12 → P25** | 「额外的 agent 贡献判断力而不是动作」正好解释了 re-agent 为什么是 planner/executor 分工 + Grok 只做复核 |
...
P25 / P25 的架构、路由、工具集、策略开关、代码规模（2,379 行 / 17 文件）全部按该仓库实测绘制。
